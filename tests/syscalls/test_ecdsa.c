                ...
                size_t tv_len)
{
  ...
  for (unsigned int i = 0; i < tv_len; i++) {
    ...
    case CX_CURVE_Stark256:
      assert_int_equal(
          cx_generic_curve((const cx_curve_weierstrass_t *)domain, ctx, &group),
          0);
      return;
      break;
    default:
...
{
  test_ecdsa(CX_CURVE_Stark256, CX_SHA256, stark_test_vector,
             sizeof(stark_test_vector) / sizeof(stark_test_vector[0]));
}
