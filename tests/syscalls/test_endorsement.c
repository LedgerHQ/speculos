...
#define os_endorsement_get_public_key_certificate sys_os_endorsement_get_public_key_certificate
...
void test_endorsement(void **state __attribute__((unused))) {
  ...
  assert_int_equal(cx_ecfp_init_public_key(CX_CURVE_256K1, raw_endorsement_pubkey, pubkey_len, &endorsement_pubkey), pubkey_len);
  assert_int_equal(cx_ecfp_init_public_key(CX_CURVE_256K1, BLUE_OWNER_PUBLIC_KEY, sizeof(BLUE_OWNER_PUBLIC_KEY), &blue_owner_endorsement_pubkey), sizeof(BLUE_OWNER_PUBLIC_KEY));

  assert_int_equal(cx_sha256_init(&sha256), CX_SHA256);
  assert_int_equal(cx_hash(&sha256.header, 0, &certif_prefix, 1, NULL, 0), 0);
  assert_int_equal(cx_hash(&sha256.header, CX_LAST, raw_endorsement_pubkey, sizeof(raw_endorsement_pubkey), hash, 32), 32);

  endorsement_sig_len = os_endorsement_get_public_key_certificate(1, endorsement_sig);
  assert_int_equal(cx_ecdsa_verify(&blue_owner_endorsement_pubkey, CX_LAST, CX_SHA256, hash, sizeof(hash), endorsement_sig, endorsement_sig_len), 1);

  assert_int_equal(os_endorsement_get_code_hash(code_hash), 32);

  assert_int_equal(cx_sha256_init(&sha256), CX_SHA256);
  assert_int_equal(cx_hash(&sha256.header, 0, nonce, sizeof(nonce), NULL, 0), 0);
  assert_int_equal(cx_hash(&sha256.header, CX_LAST, code_hash, sizeof(code_hash), hash, 32), 32);

  endorsement_key1_sig_len = os_endorsement_key1_sign_data(nonce, sizeof(nonce), endorsement_key1_sig);
  assert_int_equal(cx_ecdsa_verify(&endorsement_pubkey, CX_LAST, CX_SHA256, hash, sizeof(hash), endorsement_key1_sig, endorsement_key1_sig_len), 1);
};
