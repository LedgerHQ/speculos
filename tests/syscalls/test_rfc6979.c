...
static void test_ecdsa_rfc6979_secp256r1(void **UNUSED(state)) {
  ...
  for (size_t i = 0; i < sizeof(tvs) / sizeof(tvs[0]); i++) {
    ...
    cx_rng_rfc6979_init(&rfc_ctx, tv->digest, raw_private_key, len, digest, md_size,
                        domain->n, len);
...
static void test_ecdsa_rfc6979_secp256k1(void **UNUSED(state)) {
  ...
  assert_int_equal(sys_cx_ecfp_init_private_key(CX_CURVE_SECP256K1, raw_private_key,
                                            len, &private_key),
                   len);
