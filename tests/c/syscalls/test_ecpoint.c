...
static void assert_ecpoint_add_result(cx_curve_t curve, uint8_t *p,
                                      size_t p_len, uint8_t *q, size_t q_len,
                                      uint8_t *expected_r,
                                      size_t expected_r_len, size_t curve_len)
{
  uint8_t r[64] = { 0 };
  cx_ecpoint_t ec_p = { 0 };
  cx_ecpoint_t ec_q = { 0 };
  ...
  assert_int_equal(p_len, 2 * curve_len);
  assert_int_equal(q_len, 2 * curve_len);
  assert_int_equal(expected_r_len, 2 * curve_len);
  assert_true(sizeof(r) >= 2 * curve_len);

  assert_int_equal(sys_cx_bn_lock(curve_len, 0), CX_OK);
  assert_int_equal(sys_cx_ecpoint_alloc(&ec_p, curve), CX_OK);
  assert_int_equal(sys_cx_ecpoint_alloc(&ec_q, curve), CX_OK);
  assert_int_equal(sys_cx_ecpoint_alloc(&ec_r, curve), CX_OK);

  assert_int_equal(
      sys_cx_ecpoint_init(&ec_p, p, curve_len, p + curve_len, curve_len),
      CX_OK);
  assert_int_equal(
      sys_cx_ecpoint_init(&ec_q, q, curve_len, q + curve_len, curve_len),
      CX_OK);
  assert_int_equal(sys_cx_ecpoint_add(&ec_r, &ec_p, &ec_q), CX_OK);
  assert_int_equal(
      sys_cx_ecpoint_export(&ec_r, r, curve_len, r + curve_len, curve_len),
      CX_OK);
  assert_int_equal(sys_cx_bn_unlock(), CX_OK);
  assert_memory_equal(expected_r, r, sizeof(expected_r));
}

static void test_ecpoint_add_pallas(void **state __attribute__((unused)))
{
  uint8_t p[] = { 0x16, 0xd2, 0xcd, 0xa8, 0xef, 0x80, 0xb4, 0xc4, 0x9b, 0x3a,
                  0xda, 0xcc, 0x38, 0x4f, 0xea, 0xf1, 0xfb, 0x38, 0xa3, 0x5b,
                  0xe1, 0xbc, 0x02, 0xad, 0x6a, 0xa4, 0xc4, 0x99, 0xb6, 0xc6,
                  0xfe, 0xe6, 0x03, 0x88, 0xe7, 0x11, 0xd1, 0x20, 0xf9, 0x03,
                  0xf0, 0x1c, 0x36, 0xe2, 0x14, 0x39, 0x21, 0xeb, 0x6b, 0xc2,
                  0x7f, 0xdd, 0xd5, 0xd3, 0xb6, 0x53, 0x44, 0xc1, 0xa7, 0x67,
                  0x3d, 0x7d, 0x0d, 0x78 };
  uint8_t q[] = { 0x36, 0x18, 0x3a, 0x16, 0x36, 0xd7, 0x3b, 0xf0, 0x6f, 0x65,
                  0xd6, 0x98, 0xb4, 0x36, 0x20, 0xb3, 0xd6, 0xb8, 0xaf, 0x06,
                  0xf4, 0x4b, 0xd2, 0x0b, 0xc8, 0x02, 0xeb, 0xea, 0xa9, 0x41,
                  0x97, 0x4f, 0x00, 0x80, 0x81, 0xc8, 0x75, 0x85, 0xe4, 0x7b,
                  0x7b, 0xec, 0x78, 0xfa, 0xd5, 0x74, 0x3d, 0xf7, 0x06, 0x37,
                  0x40, 0xfa, 0xa1, 0xc5, 0xef, 0x40, 0x90, 0x23, 0x58, 0x50,
                  0xc7, 0xa8, 0x19, 0x54 };

  uint8_t expected_r[] = { 0x1f, 0xc1, 0x2c, 0xa8, 0xc1, 0x33, 0x24, 0x0e,
                           0x3e, 0x81, 0x6d, 0x4f, 0x5d, 0xbd, 0xac, 0xbb,
                           0x3f, 0x5a, 0x81, 0xbe, 0x4d, 0x0f, 0x1f, 0x53,
                           0x62, 0x37, 0x31, 0xfa, 0x26, 0xb5, 0xbd, 0x07,
                           0x19, 0x23, 0x65, 0xe8, 0x53, 0x5e, 0x65, 0x15,
                           0xa0, 0x66, 0xcd, 0x14, 0x47, 0xf2, 0xa6, 0x20,
                           0x90, 0xbb, 0x1a, 0x18, 0xe5, 0x71, 0x13, 0xe8,
                           0x46, 0x90, 0x88, 0x38, 0x7f, 0x7a, 0xea, 0xc0 };

  assert_ecpoint_add_result(CX_CURVE_PALLAS, p, sizeof(p), q, sizeof(q),
                            expected_r, sizeof(expected_r), PASTA_SIZE_u8);
}

static void test_ecpoint_scalarmul_pallas(void **state __attribute__((unused)))
{
  uint8_t point_in[] = { 0x04, 0x28, 0x48, 0x25, 0x74, 0x0b, 0x7d, 0xba, 0x08,
                         0x3b, 0x10, 0xcf, 0x18, 0x4f, 0x4e, 0x64, 0xe0, 0x34,
                         0x80, 0x39, 0x6c, 0x84, 0x96, 0xda, 0x70, 0x71, 0x4f,
                         0xe1, 0x7e, 0x2f, 0x06, 0xef, 0x07, 0x36, 0x3f, 0x4b,
                         0x44, 0xaf, 0xc6, 0x0d, 0xed, 0xd4, 0xa6, 0x16, 0x4a,
                         0x91, 0xa4, 0xbf, 0xee, 0xa5, 0xbc, 0x6c, 0x32, 0x69,
                         0x89, 0xb5, 0xf8, 0x5d, 0x00, 0x2e, 0x7b, 0x0a, 0xf4,
                         0x20, 0x7a };
  uint8_t expected_out[] = {
    0x04, 0x35, 0x62, 0x36, 0x34, 0x3f, 0xf6, 0x78, 0xc3, 0xf9, 0xf4,
    0x19, 0xc5, 0xcf, 0xc6, 0x48, 0xab, 0xc0, 0x34, 0xd8, 0x56, 0xd9,
    0x76, 0x4e, 0x7c, 0x36, 0x83, 0x06, 0x89, 0x5d, 0x26, 0x63, 0xb3,
    0x34, 0x80, 0x4f, 0xab, 0x52, 0x38, 0xb0, 0xca, 0x70, 0xc2, 0xfb,
    0xe9, 0x4d, 0xac, 0x04, 0x20, 0xbb, 0x3f, 0x11, 0xc1, 0x6b, 0xd6,
    0x3c, 0xb5, 0xe8, 0x83, 0xaf, 0x78, 0xbb, 0x84, 0x55, 0xcd
  };

  uint8_t scalar[] = { 0x1c, 0x1e, 0xc9, 0xdf, 0xd4, 0x26, 0x30, 0xea,
                       0x88, 0x8d, 0x5f, 0xc5, 0x11, 0x4a, 0xf8, 0xe8,
                       0x17, 0x83, 0x8a, 0x41, 0x83, 0x90, 0x32, 0x02,
                       0x51, 0x0c, 0x85, 0xdc, 0x88, 0x57, 0x7a, 0x76
  ...
  assert_int_equal(sys_cx_bn_lock(PASTA_SIZE_u8, 0), CX_OK);
  assert_int_equal(sys_cx_ecpoint_alloc(&P, CX_CURVE_PALLAS), CX_OK);
  assert_int_equal(sys_cx_ecpoint_init(&P, point_in + 1, PASTA_SIZE_u8,
                                       point_in + 1 + PASTA_SIZE_u8,
                                       PASTA_SIZE_u8),
                   CX_OK);
  assert_int_equal(sys_cx_ecpoint_scalarmul(&P, scalar, sizeof(scalar)), CX_OK);
  point_out[0] = 0x04;
  assert_int_equal(sys_cx_ecpoint_export(&P, point_out + 1, PASTA_SIZE_u8,
                                         point_out + 1 + PASTA_SIZE_u8,
                                         PASTA_SIZE_u8),
                   CX_OK);
  assert_int_equal(sys_cx_bn_unlock(), CX_OK);
  assert_memory_equal(expected_out, point_out, sizeof(expected_out));
...
static void test_ecpoint_add_vesta(void **state __attribute__((unused)))
{
  uint8_t p[] = { 0x1c, 0xfc, 0xea, 0x6b, 0x9d, 0xcd, 0xab, 0x97, 0x62, 0x79,
                  0x56, 0x42, 0x3e, 0x80, 0x59, 0x63, 0x07, 0x02, 0x04, 0x04,
                  0x75, 0x86, 0x86, 0xf7, 0x4a, 0x6a, 0xd1, 0x80, 0x61, 0xb2,
                  0xb3, 0xd4, 0x3a, 0x90, 0x98, 0xa9, 0xb5, 0x5f, 0xad, 0x54,
                  0x93, 0x7a, 0x7f, 0x40, 0x9b, 0x59, 0xc1, 0xfd, 0xd5, 0x6e,
                  0x3d, 0x52, 0x22, 0xeb, 0x5c, 0x6a, 0x01, 0x01, 0x52, 0x05,
                  0xd0, 0x70, 0x1f, 0x6e };
  uint8_t q[] = { 0x02, 0x53, 0xdb, 0xbc, 0xc6, 0x15, 0xa1, 0x87, 0x9e, 0x12,
                  0xa6, 0x9d, 0x60, 0x1c, 0xab, 0x8a, 0xe1, 0x5f, 0x2f, 0xfa,
                  0x1a, 0x1e, 0xce, 0xd5, 0xd4, 0x70, 0x06, 0x7a, 0xb6, 0xcb,
                  0xa0, 0x06, 0x29, 0x1e, 0x04, 0xbc, 0x91, 0xcc, 0x28, 0xae,
                  0x71, 0x8a, 0x39, 0xa0, 0x07, 0xe3, 0xc0, 0x1a, 0x2e, 0x03,
                  0x98, 0xc5, 0xd7, 0x2a, 0xb8, 0x34, 0x80, 0x06, 0xb0, 0x13,
                  0xee, 0x32, 0x87, 0x90 };

  uint8_t expected_r[] = { 0x2c, 0x63, 0x3f, 0xdf, 0x1e, 0x49, 0x07, 0xa7,
                           0xd4, 0x4d, 0x94, 0x00, 0x88, 0xd3, 0xf7, 0x67,
                           0xda, 0x28, 0x74, 0xeb, 0x16, 0x56, 0x5d, 0xb6,
                           0x69, 0x94, 0xfd, 0x9d, 0x63, 0x0d, 0x6e, 0x41,
                           0x2b, 0x92, 0x30, 0xb8, 0x12, 0xd0, 0x42, 0x89,
                           0x99, 0x1e, 0xfc, 0xc7, 0x5e, 0x23, 0xd4, 0xce,
                           0x7e, 0xcb, 0xed, 0x68, 0x86, 0x57, 0xe7, 0xbc,
                           0x0c, 0xdb, 0xa8, 0x97, 0xe4, 0x88, 0xc1, 0x83 };

  assert_ecpoint_add_result(CX_CURVE_VESTA, p, sizeof(p), q, sizeof(q),
                            expected_r, sizeof(expected_r), PASTA_SIZE_u8);
}

static void test_ecpoint_scalarmul_vesta(void **state __attribute__((unused)))
{
  uint8_t point_in[] = { 0x04, 0x3b, 0x4a, 0xcb, 0xba, 0x89, 0x60, 0xa2, 0x57,
                         0xf3, 0xe7, 0x92, 0xbe, 0x6a, 0x79, 0x4e, 0xd3, 0x6c,
                         0x34, 0x0a, 0xde, 0x31, 0x97, 0xe8, 0x12, 0x69, 0x36,
                         0x89, 0xba, 0xeb, 0x16, 0xd5, 0x33, 0x1b, 0xb8, 0xa3,
                         0x2f, 0x3b, 0xe3, 0x47, 0xe4, 0xe6, 0x6d, 0x80, 0xe4,
                         0x1a, 0x45, 0xeb, 0xf5, 0xf1, 0xb7, 0xb4, 0x93, 0x32,
                         0xe5, 0x73, 0x3e, 0xfb, 0x47, 0x1b, 0xa6, 0x83, 0xac,
                         0xff, 0x54 };
  uint8_t expected_out[] = {
    0x04, 0x0e, 0xc8, 0x51, 0x1e, 0xd1, 0x8a, 0x09, 0x32, 0x79, 0xb6,
    0xab, 0xfa, 0x01, 0x1d, 0xd9, 0x4c, 0xd8, 0x6d, 0x3f, 0x50, 0x0d,
    0x0e, 0xe5, 0x8c, 0x84, 0xc8, 0x4f, 0x5b, 0xdb, 0xf3, 0x0a, 0x88,
    0x1d, 0x7a, 0xcc, 0xb6, 0x63, 0x1e, 0xe8, 0xdb, 0x82, 0xe2, 0xdf,
    0x36, 0x07, 0xef, 0x46, 0x11, 0x99, 0x16, 0x19, 0x04, 0x40, 0x6d,
    0xd9, 0x72, 0x82, 0x5b, 0xa9, 0xfa, 0x34, 0xe6, 0x32, 0x8d
  };

  uint8_t scalar[] = { 0x20, 0x7b, 0x58, 0x59, 0xdb, 0x7c, 0x04, 0x42,
                       0x54, 0x8d, 0xf5, 0xec, 0xfe, 0xb9, 0x69, 0xec,
                       0x55, 0xae, 0xda, 0x6f, 0x6e, 0x92, 0xc8, 0x81,
                       0x80, 0x6d, 0xf9, 0xe3, 0x2b, 0xd6, 0x87, 0x2a
  ...
  assert_int_equal(sys_cx_bn_lock(PASTA_SIZE_u8, 0), CX_OK);
  assert_int_equal(sys_cx_ecpoint_alloc(&P, CX_CURVE_VESTA), CX_OK);
  assert_int_equal(sys_cx_ecpoint_init(&P, point_in + 1, PASTA_SIZE_u8,
                                       point_in + 1 + PASTA_SIZE_u8,
                                       PASTA_SIZE_u8),
                   CX_OK);
...
static void test_ecpoint_add_jubjub(void **state __attribute__((unused)))
{
  uint8_t p[] = { 0x16, 0x0a, 0x53, 0x08, 0xa9, 0x46, 0x6d, 0x47, 0x22, 0x57,
                  0x3d, 0x33, 0xb8, 0xae, 0x4d, 0x60, 0x88, 0xdd, 0xff, 0x16,
                  0x4d, 0xad, 0x57, 0x09, 0x6e, 0x05, 0x66, 0xbe, 0xed, 0x00,
                  0xf0, 0x88, 0x3c, 0x32, 0xa1, 0x53, 0x08, 0x08, 0xc3, 0x64,
                  0x82, 0xcd, 0x15, 0x88, 0x0e, 0xab, 0xef, 0x41, 0x43, 0x9f,
                  0xd0, 0x16, 0x7a, 0xc2, 0x3e, 0x40, 0x77, 0x72, 0x0f, 0x0c,
                  0x24, 0x92, 0x56, 0x8b };
  uint8_t q[] = { 0x0d, 0x85, 0xef, 0xb1, 0xed, 0x6f, 0xd5, 0xee, 0xe8, 0x44,
                  0xaf, 0x4d, 0xd0, 0x78, 0x22, 0xe6, 0xee, 0x06, 0x42, 0xa5,
                  0x38, 0x38, 0x93, 0xf4, 0xd9, 0x8a, 0x82, 0xc8, 0xfb, 0x3b,
                  0x67, 0x4e, 0x45, 0xfb, 0x4a, 0xec, 0x80, 0x00, 0x33, 0xe7,
                  0x51, 0x30, 0xe1, 0xf5, 0xb5, 0x6b, 0xc3, 0x1e, 0x75, 0xe6,
                  0xc8, 0xb6, 0x3f, 0x50, 0xcb, 0x00, 0x47, 0xa5, 0x80, 0xf8,
                  0xb0, 0x60, 0x8f, 0xab };

  uint8_t expected_r[] = { 0x2a, 0x30, 0xa2, 0x55, 0xcf, 0x67, 0xb3, 0xbf,
                           0xfb, 0x64, 0x7f, 0xbe, 0x60, 0x36, 0x78, 0x8d,
                           0xdb, 0x41, 0xa8, 0xc7, 0x6e, 0xd8, 0x96, 0x08,
                           0x4e, 0x30, 0x51, 0x59, 0x0f, 0xed, 0x84, 0xa6,
                           0x0e, 0x08, 0x7a, 0x73, 0x48, 0xda, 0x7e, 0x61,
                           0x02, 0xe0, 0xc7, 0x1a, 0xd1, 0x4f, 0x1d, 0xfa,
                           0x99, 0x26, 0x2b, 0x5a, 0x86, 0xaa, 0xef, 0x65,
                           0x77, 0x82, 0x07, 0x87, 0x3f, 0xa7, 0x78, 0xc1 };

  assert_ecpoint_add_result(CX_CURVE_JUBJUB, p, sizeof(p), q, sizeof(q),
                            expected_r, sizeof(expected_r), PASTA_SIZE_u8);
}

static void test_ecpoint_scalarmul_jubjub(void **state __attribute__((unused)))
{
  uint8_t point_in[] = { 0x04, 0x35, 0x08, 0x9a, 0xeb, 0x84, 0x0b, 0xd9, 0x22,
                         0xcb, 0x72, 0x81, 0x17, 0x50, 0x8f, 0xf3, 0x8b, 0x03,
                         0x3b, 0x3f, 0x11, 0x0c, 0x09, 0x15, 0x9a, 0x88, 0x23,
                         0xf7, 0xe9, 0x02, 0x58, 0x59, 0x28, 0x55, 0x16, 0xcf,
                         0x42, 0xa9, 0x1c, 0xac, 0x6f, 0x53, 0x5b, 0x86, 0xbd,
                         0xbe, 0x4b, 0x0d, 0x98, 0xf7, 0x41, 0x78, 0xc3, 0x0b,
                         0x04, 0x66, 0x7b, 0xcb, 0x00, 0x84, 0x5f, 0x41, 0xbc,
                         0xe7, 0xb8 };
  uint8_t expected_out[] = {
    0x04, 0x33, 0xf7, 0x75, 0x5b, 0x10, 0xd9, 0x13, 0x8b, 0x55, 0x9f,
    0x3c, 0xae, 0x00, 0xc1, 0x7f, 0x2a, 0x1a, 0x53, 0x52, 0x49, 0x15,
    0xa9, 0x50, 0x15, 0x74, 0xb5, 0x91, 0x25, 0x2d, 0xc6, 0x5c, 0xfd,
    0x09, 0x35, 0x66, 0xb9, 0xdb, 0x44, 0x51, 0x63, 0xd6, 0x7a, 0x95,
    0x34, 0x59, 0x1d, 0xb3, 0x93, 0xc7, 0x66, 0x23, 0x9b, 0x48, 0x78,
    0x7f, 0xdf, 0x78, 0x38, 0xab, 0x1e, 0x3a, 0xea, 0x41, 0xcf
  };

  uint8_t scalar[] = { 0xe0, 0x5e, 0xb7, 0xa2, 0xdb, 0x60, 0xd2, 0x50,
                       0xf6, 0xd7, 0x8d, 0x87, 0x80, 0x61, 0xc4, 0x0b,
                       0xb0, 0x48, 0x21, 0xf0, 0xc5, 0x72, 0xff, 0x59,
                       0x92, 0xeb, 0x5d, 0x03, 0x93, 0x47, 0xf5, 0x8d };
  ...
  assert_int_equal(sys_cx_bn_lock(PASTA_SIZE_u8, 0), CX_OK);
  assert_int_equal(sys_cx_ecpoint_alloc(&P, CX_CURVE_JUBJUB), CX_OK);
  assert_int_equal(sys_cx_ecpoint_init(&P, point_in + 1, PASTA_SIZE_u8,
                                       point_in + 1 + PASTA_SIZE_u8,
                                       PASTA_SIZE_u8),
                   CX_OK);
...
int main(void)
{
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(test_ecpoint_add_pallas),
    cmocka_unit_test(test_ecpoint_scalarmul_pallas),
    cmocka_unit_test(test_ecpoint_add_vesta),
    cmocka_unit_test(test_ecpoint_scalarmul_vesta),
    cmocka_unit_test(test_ecpoint_add_jubjub),
    cmocka_unit_test(test_ecpoint_scalarmul_jubjub),
    /* clang-format on */
  };
