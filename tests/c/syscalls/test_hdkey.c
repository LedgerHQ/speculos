...
static void setup_context_result(HDKEY_params_t *params, uint8_t *private_key,
                                 size_t private_key_len, uint8_t *chain,
                                 size_t chain_len)
{
...
static void test_HDKEY_derive_zip32_sapling(void **state)
{
  ...
  size_t offset = 0;
  // ask
  uint8_t ask[HDKEY_ZIP32_KEY_LEN] = { 0xcc, 0xb7, 0xb3, 0x60, 0xc6, 0x49, 0x14,
                                       0xa0, 0xdd, 0x0d, 0xa7, 0xb7, 0x10, 0xfa,
                                       0x4f, 0x85, 0x17, 0x01, 0x37, 0xb3, 0x36,
                                       0xe0, 0x84, 0x5c, 0x24, 0x00, 0x38, 0xa8,
                                       0x68, 0x7b, 0xbf, 0x03 };
  // nsk
  uint8_t nsk[HDKEY_ZIP32_KEY_LEN] = { 0x87, 0x6c, 0x89, 0x37, 0x0a, 0x70, 0x42,
                                       0x31, 0xc9, 0xba, 0x1c, 0x19, 0x8f, 0x9f,
                                       0xbf, 0xe6, 0x11, 0x61, 0x93, 0x86, 0x76,
                                       0x1f, 0xe6, 0x78, 0x36, 0x50, 0x3f, 0x21,
                                       0xac, 0x23, 0xca, 0x0a };
  // ovk
  uint8_t ovk[HDKEY_ZIP32_KEY_LEN] = { 0xf7, 0x68, 0xf4, 0x6d, 0x04, 0x1d, 0xd9,
                                       0x3c, 0x85, 0x22, 0xd1, 0x25, 0x3d, 0x9c,
                                       0xb9, 0xd7, 0x74, 0xf0, 0x44, 0xbd, 0x43,
                                       0xe2, 0xb4, 0x54, 0x87, 0x85, 0x27, 0x49,
                                       0xf4, 0xe6, 0x8d, 0xb9 };
  // dk
  uint8_t dk[HDKEY_ZIP32_KEY_LEN] = { 0x5f, 0x76, 0x07, 0x62, 0x6a, 0x2e, 0xdf,
                                      0xd6, 0x4d, 0x20, 0x44, 0x2c, 0xdb, 0xe8,
                                      0x1a, 0x4c, 0x09, 0x83, 0xc9, 0xa9, 0x07,
                                      0xda, 0x53, 0x20, 0x29, 0x57, 0xb8, 0x53,
                                      0xb3, 0x80, 0x08, 0x86 };
  // c
  uint8_t cc[HDKEY_ZIP32_KEY_LEN] = { 0x7f, 0x59, 0x4c, 0x97, 0x2b, 0x75, 0x73,
                                      0xb1, 0xb9, 0x2d, 0x49, 0x89, 0x63, 0xf3,
                                      0xd5, 0xf5, 0xc2, 0xec, 0x59, 0x86, 0x8a,
                                      0x71, 0x68, 0x0b, 0x98, 0x6d, 0x63, 0x0f,
                                      0xd2, 0x14, 0xc9, 0x45 };

  // Set up context
  setup_context(&params, BOLOS_TRUE, HDKEY_DERIVE_MODE_ZIP32_SAPLING,
                CX_CURVE_JUBJUB, test_path, ARRAYLEN(test_path));
  // Set up result pointers
  setup_context_result(&params, private_key, sizeof(private_key), chain,
                       sizeof(chain));
...
static void test_HDKEY_derive_zip32_orchard(void **state)
{
  ...
  HDKEY_params_t params = { 0 };
  // sk
  uint8_t sk[HDKEY_ZIP32_KEY_LEN] = { 0x51, 0xf9, 0xed, 0x7c, 0xb2, 0xba, 0x22,
                                      0x0a, 0xc9, 0xf4, 0x9c, 0xad, 0x4d, 0xcc,
                                      0x50, 0xe0, 0x0d, 0x62, 0x56, 0x75, 0xba,
                                      0x1f, 0x39, 0xc8, 0xc1, 0x16, 0x55, 0xa8,
                                      0xc5, 0x73, 0xf4, 0xae };
  // chain code
  uint8_t cc[HDKEY_ZIP32_KEY_LEN] = { 0x92, 0x4c, 0xa8, 0x16, 0xac, 0xf1, 0x06,
                                      0xd7, 0x38, 0x9a, 0x59, 0x12, 0xcc, 0x77,
                                      0xc6, 0x3e, 0x2b, 0x76, 0xc0, 0x41, 0xc3,
                                      0x18, 0x9f, 0x25, 0x99, 0x4b, 0xdf, 0x4d,
                                      0x4a, 0x53, 0x62, 0x55 };

  // Set up context
  setup_context(&params, BOLOS_TRUE, HDKEY_DERIVE_MODE_ZIP32_ORCHARD,
                CX_CURVE_NONE, test_path, ARRAYLEN(test_path));
  // Set up result pointers
  setup_context_result(&params, private_key, sizeof(private_key), chain,
                       sizeof(chain));
...
static void test_HDKEY_derive_zip32_registered(void **state)
{
  ...
  // len(tag_seq_1) || tag_seq_1 || tag || len(tag_seq_2) || tag_seq_2
  ...
  // Set up context
  setup_context(&params, BOLOS_TRUE, HDKEY_DERIVE_MODE_ZIP32_REGISTERED,
                CX_CURVE_NONE, test_path, ARRAYLEN(test_path));
  // Set up result pointers
  setup_context_result(&params, private_key, sizeof(private_key), chain,
                       sizeof(chain));
...
static void test_HDKEY_derive_bls12377(void **state)
{
  ...
  //     seed:
  //     '2e6695fe0bd1e628215e5f8c0aa6b374233c6fa06019057d6d786ae66d937ef5'
  // },
  ...
  uint8_t expected_key[] = { 0x2e, 0x66, 0x95, 0xfe, 0x0b, 0xd1, 0xe6, 0x28,
                             0x21, 0x5e, 0x5f, 0x8c, 0x0a, 0xa6, 0xb3, 0x74,
                             0x23, 0x3c, 0x6f, 0xa0, 0x60, 0x19, 0x05, 0x7d,
                             0x6d, 0x78, 0x6a, 0xe6, 0x6d, 0x93, 0x7e, 0xf5 };
  uint8_t expected_chain[] = { 0xc0, 0xcb, 0xe8, 0xcc, 0xe5, 0xb2, 0x9f, 0x2a,
                               0x78, 0x7e, 0x1f, 0x77, 0x91, 0x5a, 0xf2, 0xc2,
                               0xe7, 0x1f, 0xe6, 0x5d, 0x4d, 0xcb, 0x7a, 0xd7,
                               0x97, 0xb3, 0x58, 0x4f, 0xcb, 0x42, 0xbc, 0xb5 };

  memset(&params, 0, sizeof(HDKEY_params_t));

  // Set up context
  setup_context(&params, BOLOS_TRUE, HDKEY_DERIVE_MODE_BLS12377_ALEO,
                CX_CURVE_BLS12_377_G1, path, ARRAYLEN(path));
  // Set up result pointers
  setup_context_result(&params, private_key, sizeof(private_key), chain,
                       sizeof(chain));
  // Init seed
