...
void test_aes_cbc1(void **state __attribute__((unused))) {
  ...
  const uint8_t expected[CX_AES_BLOCK_SIZE * 3] = {
    0x3c, 0x8c, 0x4b, 0xa, 0xa2, 0x1b, 0x8, 0x5c, 0x53, 0x8a, 0x99, 0x45, 0x79, 0x14, 0x2f, 0x24,
    0xe9, 0x5f, 0x3c, 0xf9, 0xa9, 0xd0, 0x3d, 0xc7, 0xa, 0xaa, 0x65, 0x6d, 0xc8, 0xcb, 0xb7, 0x7b,
    0xb7, 0xcc, 0x42, 0xcf, 0x7d, 0xf0, 0x51, 0x74, 0xf6, 0xde, 0x25, 0xb9, 0x8f, 0x94, 0x63, 0x9b
  };
  ...
  ret = sys_cx_aes(&key, CX_LAST | CX_ENCRYPT | CX_PAD_NONE | CX_CHAIN_CBC, in, sizeof(in), out, sizeof(out));
  assert_int_equal(ret, sizeof(in));

  assert_memory_equal(out, expected, sizeof(expected));

  ret = sys_cx_aes(&key, CX_LAST | CX_DECRYPT | CX_PAD_NONE | CX_CHAIN_CBC, out, sizeof(out), out, sizeof(out));
...
void test_aes_ecb1(void **state __attribute__((unused))) {
  ...
  const uint8_t expected[CX_AES_BLOCK_SIZE * 3] = {
    0x3c, 0x8c, 0x4b, 0xa, 0xa2, 0x1b, 0x8, 0x5c, 0x53, 0x8a, 0x99, 0x45, 0x79, 0x14, 0x2f, 0x24,
    0x5a, 0x54, 0xef, 0x77, 0xf4, 0x9f, 0x28, 0x5e, 0x48, 0xe2, 0xf5, 0x38, 0x86, 0x3e, 0xc4, 0x30,
    0xce, 0x9b, 0xb1, 0x56, 0x7a, 0xf6, 0x48, 0xbb, 0xd1, 0x72, 0xc8, 0x6b, 0xd5, 0xbf, 0x9c, 0xa7
  };
  uint8_t rawkey[CX_AES_BLOCK_SIZE];
  int ret;
  ...
  ret = sys_cx_aes(&key, CX_LAST | CX_ENCRYPT | CX_PAD_NONE | CX_CHAIN_ECB, in, sizeof(in), out, sizeof(out));
  assert_int_equal(ret, sizeof(in));
