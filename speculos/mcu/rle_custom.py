...
class RLECustomBase:
    ...
    def bpp1_to_values(data):
        ...
        if len(output) != (len(data) * 8):
            raise ValueError("Output length does not match expected length")
    ...
    def encode_pass2(pairs, max_count=64):
        ...
        while index < len(single_output):
            ...
            if (byte & 0xF0) != 0x00:
                ...
                count = 1
                while ((index + count) < len(single_output)) and (single_output[index + count] & 0xF0) == 0x00:
                    count += 1
    ...
    def get_encoded_size(self, data):
        ...
        for command, repeat, pixels in data:
            ...
            if command == self.CMD_FILL:
                ...
                if repeat != len(pixels):
                    raise ValueError("Repeat count does not match the number of pixels")
        ...
        sys.stdout.write(f"sizes: cmd={cmd_size}, count={count_size}, data={pixels_size}\n")
...
if __name__ == "__main__":
    ...
    def main(args):
        ...
        with RLECustomA(args.bpp, args.verbose) as rle:
            ...
            sys.stdout.write(f"Encoded size: {encoded_size} bytes (instead of {len(data)})\n")
    ...
    # -------------------------------------------------------------------------
    # Parse arguments:
    parser = argparse.ArgumentParser(description="Test custom RLE methods (Build #220223.1003)")

    parser.add_argument(
        "-b", "--bpp", dest="bpp", type=int, default=1, help="Number of bits per pixel ('%(default)s' by default)"
    )

    parser.add_argument("-v", "--verbose", action="store_true", help="Add verbosity to output ('%(default)s' by default)")
