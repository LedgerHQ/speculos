...
def get_json_font(json_name:str) -> Mapping[Char, BitMapChar]:
    ...
    # Build the filename by adding api level information
    json_name += f"-api-level-{OCR.api_level}.json"
    # Now add the fonts path (JSON files are in speculos/fonts)
    script_path = os.path.abspath(os.path.dirname(__file__))
    json_name = os.path.join(script_path, "..", "fonts", json_name)
...
class OCR:
    api_level = 0
    def __init__(self, api_level=None):
        self.events: List[TextEvent] = []
        if api_level is not None:
            # Save the API_LEVEL in a class variable
