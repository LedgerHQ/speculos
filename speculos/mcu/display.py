...
class FrameBuffer:
    ...
    def check_color(self, color: int) -> int:
        ...
        if self.model != 'stax' and self.model != 'europa':
            if color != 0x000000:
    ...
    def get_public_screenshot(self) -> bytes:
        if self.model == "stax" or self.model == "europa":
            # On Stax/Europa, we only make the screenshot public on the RESTFUL api when it is consistent with events
            # On top of this, take_screenshot is time consuming on stax/europa, so we'll do as few as possible
