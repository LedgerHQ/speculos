...
def get_sharedlib_infos(app_path, apiLevel):
    with open(app_path, 'rb') as fp:
        elf = ELFFile(fp)
        text = elf.get_section_by_name('.text')
        # The name of the section is shram, starting at API Level 22
        if int(apiLevel) < 22:
        ...
        fonts_size = 0
        pic_init_addr = 0
        # At API Level 22, fonts are stored in shared elf, in C_nbgl_fonts variable
        if int(apiLevel) >= 22:
            symtab = elf.get_section_by_name('.symtab')
            ...
            if nbgl_fonts_symbol is not None:
                ...
                logger.info("Disabling OCR.")
            # At API Level >= 22, a function called pic_init() needs to be retrieved
...
def run_qemu(s1: socket.socket, s2: socket.socket, args: argparse.Namespace, use_bagl: bool) -> int:
    ...
    # for NBGL apps, fonts binary file is mandatory before API Level 22
    if not use_bagl and int(args.apiLevel) < 22:
