...
With applications built by recent SDKs, Speculos can automatically detect the targeted device. Otherwise, the Nano S is the default; the Nano X, Nano S+, Europa, Stax and Blue can be specified on the command line:
...
./speculos.py --model europa apps/btc.elf.elf
```
...
OCR is available for NanoX, Nanos S+, Europa and Stax with built in character recognition.
