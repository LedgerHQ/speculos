...
// This algorithm enables to generate a 32 bytes BLS private key
// It intends to replace the BIP32 HD derivation for BLS12-381 signatures since the generated 
...
static void cx_parent_sk_to_lamport_pk(const unsigned char *parent_sk, unsigned int index, unsigned char *compressed_lamport_pk) {
  ...
  for (i = 0; i < 2 ; i++) {
    ...
    cx_hkdf_extract(CX_SHA256, local_sk, sizeof(local_sk), salt, sizeof(salt), prk);
    ...
    for (j = 1; j < 256; j++) {
      ...
      sys_cx_hmac(&hmac_ctx, CX_LAST | CX_NO_REINIT, &block, 1, tmp, CX_SHA256_SIZE);
      sys_cx_hash_sha256(tmp, CX_SHA256_SIZE, tmp2, CX_SHA256_SIZE);
      sys_cx_hash((cx_hash_t*)&sha256, 0, tmp2, CX_SHA256_SIZE, NULL, 0);
      block++;
    }
  }
  sys_cx_hash((cx_hash_t*)&sha256, CX_LAST, NULL, 0, compressed_lamport_pk, 32);
...
// Output: sk = the secret key, an integer s.t 0 <= sk < r, r is the subgroup order
...
static void cx_hkdf_mod_r(const unsigned char *ikm, unsigned int ikm_len, unsigned char *key_info,
                              unsigned int key_info_len, unsigned char *sk) {
  unsigned char             prk[CX_SHA256_SIZE];
...
static void cx_derive_master_sk(const unsigned char *seed, unsigned int seed_len, unsigned char *sk) {
...
static int cx_derive_child_sk(const unsigned char *parent_sk, const unsigned int *path, unsigned int path_len, unsigned char *child_sk) {
  ...
  for (i = 0; i < path_len; i++) {
    ...
    cx_parent_sk_to_lamport_pk((i == 0 ? parent_sk : child_sk), path[i], lamport_pk);
