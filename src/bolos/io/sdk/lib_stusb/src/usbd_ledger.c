...
void USBD_LEDGER_start(void)
{
    ...
    if ((usbd_ledger_data.state == USBD_LEDGER_STATE_INITIALIZED)
        ...
        strlcpy(usbd_ledger_data.name, usbd_ledger_init_data.name, sizeof(usbd_ledger_data.name));
        ...
        if (usbd_ledger_init_data.class_mask & USBD_LEDGER_CLASS_HID_U2F) {
            ...
            buffer[1] = usbd_ledger_init_data.hid_u2f_settings.major_device_version_number;
            buffer[2] = usbd_ledger_init_data.hid_u2f_settings.minor_device_version_number;
            buffer[3] = usbd_ledger_init_data.hid_u2f_settings.build_device_version_number;
            USBD_LEDGER_setting(
