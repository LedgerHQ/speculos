...
#ifdef HAVE_BAGL
static void os_io_seph_ux_display_bagl_icon(const bagl_component_t    *icon_component,
    ...
    if (type != BAGL_NONE) {
        if (txt != NULL) {
            ...
            if (type == BAGL_ICON && el->component.icon_id == 0) {
                ...
                os_io_seph_ux_display_bagl_icon(&el->component, (const bagl_icon_details_t *) txt);
            }
            else {
                ...
                // automated testing io_seph_send crash when using txt from language packs...
...
void io_seph_ux_send_nbgl_serialized(nbgl_serialized_event_type_e event, nbgl_obj_t *obj)
{
    // Serialize object
    size_t  len    = 0;
    uint8_t status = nbgl_serializeNbglEvent(
        event, obj, nbgl_serialize_buffer, &len, sizeof(nbgl_serialize_buffer));
