 ...
 * @param[in] result_len Length of the result buffer. It must be of size
 * HDKEY_BIP32_ED25519_KEY_LEN if the curve is Ed25519, otherwise
 * CX_SHA512_SIZE.
 * @return os_result_t
 * @retval OS_SUCCESS Success
 * @retval OS_ERR_BAD_PARAM Invalid parameter
 */
static os_result_t hdkey_bip32_check_result_len(cx_curve_t curve,
                                                size_t result_len)
{
...
/**
 * @brief Computes a HMAC of a seed stored in NVM using either a given key
 * string or the default key string.
 * @param[in] params Pointer to the HDKEY structure.
                                  ...
                                  size_t result_len)
{
  ...
  if ((os_error = hdkey_bip32_check_result_len(params->curve, result_len)) !=
      OS_SUCCESS) {
    goto end;
  }
  ...
  if (params->curve == CX_CURVE_Ed25519 && params->mode == HDW_NORMAL) {
    ...
    sys_cx_hmac_sha256(params->seed_key, params->seed_key_len, result, 65,
                       result + CX_SHA512_SIZE, CX_SHA256_SIZE);
  }

  env_get_seed(result, 64);
  sys_cx_hmac_sha512(params->seed_key, params->seed_key_len, result,
                     CX_SHA512_SIZE, result, CX_SHA512_SIZE);
...
/**
 * @brief Recomputes seed HMAC for Ed25519 BIP32 derivation if the result
 ...
 * @param[in,out] result Pointer to the result buffer of length
 * #HDKEY_BIP32_ED25519_KEY_LEN. Result length must be checked before calling
 * this function.
 * @return os_result_t
 * @retval OS_SUCCESS Success
 * @retval OS_ERR_CRYPTO_INTERNAL Cryptographic computation failure
 */
static os_result_t hdkey_bip32_ed25519_hdw_normal_seed_hmac_final(
    const uint8_t *seed_key, size_t seed_key_len, uint8_t *result)
{
  while (result[31] & 0x20) {
    if (sys_cx_hmac_sha512(seed_key, seed_key_len, result, CX_SHA512_SIZE,
                           result, CX_SHA512_SIZE) != CX_SHA512_SIZE) {
...
/**
 * @brief Recomputes seed HMAC for BIP32 derivation (except Ed25519) if the
 * result doesn't have the expected format.
 * @param[in] curve Curve identifier. Must not be Ed25519.
 * @param[in] seed_key Pointer to the HMAC default key of length seed_key_len.
 * @param[in] seed_key_len Length of the HMAC default key.
 * @param[in] seed_len Seed length.
 * @param[in,out] result Pointer to the result buffer of length #CX_SHA512_SIZE.
 * Result length must be checked before calling this function.
 * @return os_result_t
 * @retval OS_SUCCESS Success
 * @retval OS_ERR_BAD_PARAM Invalid parameter
 * @retval OS_ERR_CRYPTO_INTERNAL Cryptographic computation failure
 */
static os_result_t hdkey_bip32_hdw_normal_seed_hmac_final(
    cx_curve_t curve, const uint8_t *seed_key, size_t seed_key_len,
    size_t seed_len, uint8_t *result)
{
  int diff = 0;
                                      ...
                                      uint8_t *seed_hmac, size_t seed_hmac_len)
{
  ...
  if ((os_error = hdkey_bip32_check_result_len(params->curve, seed_hmac_len)) !=
      OS_SUCCESS) {
    goto end;
  }

  // OS checks not required here
  os_error = HDKEY_BIP32_compute_seed_hmac(params, HDKEY_BIP32_NVM_SEED,
                                           seed_hmac, seed_hmac_len);
