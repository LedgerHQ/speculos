                                              ...
                                              size_t seed_chain_len)
{
    ...
    for (size_t i = 0; i < params->path_len; i++) {
        if ((os_error = hdkey_bls12377_derive_child(params->path[i], seed_chain, seed_chain_len))
            != OS_SUCCESS) {
            return os_error;
        }
    }
...
os_result_t HDKEY_BLS12377_derive(HDKEY_params_t *params)
{
    ...
    if ((params == NULL) || (params->private_key == NULL) || (params->path == NULL)) {
        return os_error;
    }
    ...
    if ((params->chain != NULL) && (params->chain_len != HDKEY_BLS12377_KEY_LEN)) {
        os_error = OS_ERR_SHORT_BUFFER;
        goto end;
    }
    ...
    if ((os_error = HDKEY_BIP32_get_seed_hmac(params, seed_chain, sizeof(seed_chain)))
        != OS_SUCCESS) {
        goto end;
    }
    ...
    if ((os_error = hdkey_bls12377_derive_path(params, seed_chain, sizeof(seed_chain)))
        != OS_SUCCESS) {
        goto end;
    }
    ...
    if (params->chain != NULL) {
        memcpy(params->chain, &seed_chain[HDKEY_BLS12377_KEY_LEN], HDKEY_BLS12377_KEY_LEN);
    }
