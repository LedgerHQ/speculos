...
/***************** No HROW VERSION *********/
/* the following functions use errors handling instead of throwing exception, no_throw
 * versions should be preferably used*/
...
uint32_t * 	info
	  ...
	  BN_bn2binpad(r, buf_r, domain->length);
	  BN_bn2binpad(normalized_s, buf_s, domain->length);
	  *sig_len = spec_cx_ecfp_encode_sig_der(sig, 2*domain->length, buf_r, domain->length,
	                                        buf_s, domain->length);
	  free(buf_r);
	  free(buf_s);
	  ECDSA_SIG_free(ecdsa_sig);
