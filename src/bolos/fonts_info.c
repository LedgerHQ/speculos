                 ...
                 bool use_nbgl)
{
  ...
  if (use_nbgl) {
    // With NBGL apps, before API level 22, fonts are loaded at a known location, in the OS
    if (sdk_version < SDK_API_LEVEL_22) {
      switch (hw_model) {
      case MODEL_STAX:
    ...
    // starting at API level 22, fonts are in shared elf, at fonts_addr address
    else {
