...
// Fonction used by qsort/bsearch to quickly sort/find bitmap_char pairs
int compare_bitmap_char(const void *ptr1, const void *ptr2)
{
  return ((const BITMAP_CHAR *)ptr1)->bitmap - ((const BITMAP_CHAR *)ptr2)->bitmap;
}
...
uint32_t get_character_from_bitmap(const uint8_t *bitmap) {
  ...
  result = bsearch(&value, bitmap_char, nb_bitmap_char, sizeof(bitmap_char[0]), compare_bitmap_char);
...
static void parse_nbgl_font(nbgl_font_t *nbgl_font) {
  ...
  for (uint32_t c = nbgl_font->first_char; c <= nbgl_font->last_char; c++, characters++) {
...
static void parse_bagl_font(bagl_font_t *bagl_font, void *code, unsigned long text_load_addr) {
  uint8_t *bitmap = remap_addr(code, (uint32_t)bagl_font->bitmap, text_load_addr);
  bagl_font_character_t *characters = remap_addr(code, (uint32_t)bagl_font->characters, text_load_addr);

  for (uint32_t c = bagl_font->first_char; c <= bagl_font->last_char; c++, characters++) {
    // Be sure data is coherent
...
static void parse_bagl_font_5(bagl_font_t_5 *bagl_font, void *code, unsigned long text_load_addr) {
  uint8_t *bitmap = remap_addr(code, (uint32_t)bagl_font->bitmap, text_load_addr);
  bagl_font_character_t_5 *characters = remap_addr(code, (uint32_t)bagl_font->characters, text_load_addr);
  uint32_t last_character = bagl_font->last_char - bagl_font->first_char;
  uint32_t bitmap_len = characters[last_character].bitmap_offset + characters[last_character].bitmap_byte_count;
...
static void parse_bagl_font_1(bagl_font_t_1 *bagl_font, void *code, unsigned long text_load_addr) {
  uint8_t *bitmap = remap_addr(code, (uint32_t)bagl_font->bitmap, text_load_addr);
  bagl_font_character_t_1 *characters = remap_addr(code, (uint32_t)bagl_font->characters, text_load_addr);
  uint32_t last_character = bagl_font->last_char - bagl_font->first_char;
  uint32_t bitmap_len = characters[last_character].bitmap_offset + characters[last_character].bitmap_byte_count;
...
void parse_fonts(void *code, unsigned long text_load_addr, unsigned long fonts_addr, unsigned long fonts_size) {
  ...
  // Sort all those bitmap/character pairs (should be quick, they are almost sorted)
  qsort(bitmap_char, nb_bitmap_char, sizeof(bitmap_char[0]), compare_bitmap_char);
}
